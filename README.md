...
4. 选择预计到达时间，或填写一句回复
5. 若确认是自己的车，点击「我已知晓，正在前往」
6. 只有确认后，才可点击「已挪车完成」
7. 若不是自己的车，可点击「不是我的车 / 车码不匹配」；该请求进入最终状态，不能再确认/完成
8. 如遇恶意反复扫码，可点击「拉黑此扫码设备」；当前请求会反馈对方核对车牌、车辆位置和二维码，后续 30 天禁止此设备再次通知
...
- `waiting → confirmed`：车主确认正在前往
- `confirmed → completed`：车主确认后才能标记完成
- `waiting → rejected`：车码不匹配 / 不是我的车 / 拉黑设备都会让当前请求进入拒绝态
- `rejected` 和 `completed` 是最终状态，不能再改回确认、完成或车码不匹配
- 设备拉黑不是提交者页面的独立状态；提交者只看到“请核对车牌、车辆位置和二维码”，避免一直等待
...
# 产品核心流程测试：请求隔离、token 校验、状态流转、车主回复、车码不匹配、设备拉黑
node tests/movecar.test.js

# 前端计时器测试：3 分钟后显示电话、车主回应后不再显示
node tests/frontend-timer.test.js
```
...
- ⚠️ 新增“不是我的车 / 车码不匹配”状态，请求者会看到核对车牌、车辆位置和二维码提示
- 🧯 rejected 状态不再鼓励“再次通知”，改为提示先核对车辆信息
- ☎️ 备用电话入口改为 3 分钟无回应后才显示，并增加二次确认
- 🧯 修复车主已回应后旧计时器仍可能重新显示电话入口的边界问题
- 🧯 修复“车码可能不匹配”状态下提交者页面仍显示“马上到”的文案错乱
- 🧯 车码不匹配状态强制使用专用文案，避免复用车主回复框里的到达类回复
- 🧱 增加请求状态机约束：完成必须先确认，车码不匹配/完成后禁止再切换状态
- 🧼 车主处理台会根据已有状态禁用无效按钮，避免重复点击造成状态串扰
- 🧪 自动测试覆盖车主回复、车码不匹配状态、状态机非法转移、3 分钟电话计时器和拒绝态 UI
