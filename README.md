...
6. 只有确认后，才可点击「已挪车完成」
7. 若不是自己的车，可点击「不是我的车 / 误扫码」；该请求进入最终状态，不能再确认/完成
8. 如遇恶意反复扫码，可点击「拉黑此扫码设备」；当前请求会反馈对方核对车牌/二维码，后续 30 天禁止此设备再次通知
...
- `waiting → rejected`：误扫码 / 不是我的车 / 拉黑设备都会让当前请求进入拒绝态
- `rejected` 和 `completed` 是最终状态，不能再改回确认、完成或误扫码
- 设备拉黑不是提交者页面的独立状态；提交者只看到“请核对车牌/二维码”，避免一直等待
...
- 🧯 修复“可能扫错了”状态下提交者页面仍显示“马上到”的文案错乱
- 🧯 误扫码状态强制使用专用文案，避免复用车主回复框里的到达类回复
- 🧱 增加请求状态机约束：完成必须先确认，误扫码/完成后禁止再切换状态
- 🧼 车主处理台会根据已有状态禁用无效按钮，避免重复点击造成状态串扰
- 🧪 自动测试覆盖车主回复、误扫码状态、状态机非法转移、3 分钟电话计时器和误扫码 UI
